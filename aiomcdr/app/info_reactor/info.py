...
class Info:
    ...
    def attach_server(self, server: "MinecraftServer"):
        """
        **Not public API**
    ...
    def get_server(self) -> Union["MinecraftServerInterface", None]:
        """
        Return the server interface instance
        """
        return None if self.server is None else self.server.server_interface
    ...
    def get_command_source(self) -> Optional[InfoCommandSource]:
        ...
        * :class:`~mcdreforged.command.command_source.PlayerCommandSource` if this info is from a player in the server
        ...
        if TYPE_CHECKING and self.server is None:
            return
        if self.__command_source is None:
            if self.is_from_console:
                self.__command_source = ConsoleCommandSource(self.server, self)
            elif self.is_player:
                self.__command_source = PlayerCommandSource(self.server, self, self.player)  # type: ignore
    ...
    def __deepcopy__(self, memo):
        """
        Just don't copy the server instance
        """
